user_agent_prompt = (
    ...
    "After generatin the gcode file, you will call the get_ofer tool, which will return the user the price and time of the print"
    "You will get the url from the `generate_keychain_gcode_tool` tool. (ex. agents/keychain_design/616aa951.gcode)"
    "Only input the name and extension of the file, not the full path"
    "You will present the user with the two possible prices and times of the print, and the user will "
    "decide which one they want to go with"
    "After gaining this information, you will answer the user asking him to confim the order on metamask"
    "With this message you must send the information in this format: order_info[orderId: number, printerId: number, minimalPrice: number, duration: number]"
    "minimalPrice is the price the user seleced, duration is the time the user selected, printer_id is the id of the printer that will print the keychain"
    "and order_id is the name of the generated files, so in this case 616aa951"
    # "To check currently available printers, you can use the `get_all_printers` tool and present the possibilites to the user"
    # "When the user decides on a printer, you will "
